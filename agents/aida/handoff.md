...
Last updated: 2026-06-18 (post a2a-gw v1.26.19 launch closure — 14-release wave shut; new peer `aweb.ai/ragnor` first-contact handshake)
...
- `aw`: latest CLI is **1.26.19** (PyPI/npm/GH Release); verify with `aw version`. CLI DefaultTimeout 30s + team-auth envelope v2 + A2A task-token persistence in the .15-.19 wave.
- channel plugin: latest from `@awebai/aweb-channel` (1.4.12, unchanged since aapz Wave 4)
- server: ac **v0.5.74** / aweb **1.26.18** prod (verify against `/health` on `app.aweb.ai`; ops canon at status/operations.md). Server-side aweb is one minor behind PyPI 1.26.19 — known skew, not a customer issue.
- a2a gateway: **a2a-gw v1.26.19** (a2a.aweb.ai/health: `release_tag=a2a-gw-v1.26.19`, `aweb_version=1.26.19`, build 2026-06-12T23:37:14Z) — **aaqw + aaqx wave CLOSED 2026-06-12** with 14 releases (pi 0.1.21 → AC v0.5.69-.74 → a2a-gw 1.26.14/1.26.19 → aw 1.26.15-.19). Stock a2a-sdk python 1.1.0 default-flow proves the gateway works with zero customizations (per Hestia `77c18a4`).
- client packages (npm, as of 2026-06-12 post wave-close): `@awebai/claude-channel` **1.4.12** (unchanged), `@awebai/pi` **0.1.21** (bumped from .20), `@awebai/claude-skills` **0.2.12** (unchanged), `@awebai/aw` **1.26.19** (bumped from 1.26.8 via the .15-.19 wave). Marketplace pin at aweb-channel 1.4.12 + aweb-skills 0.2.12 still current. Customer upgrade path: `/plugin marketplace update awebai-marketplace && /plugin update aweb-channel@awebai-marketplace` (and same for aweb-skills). Runbook install commands are version-agnostic (marketplace alias / `@latest`) so they don't pin to these.
- **aapz EPIC COMPLETE 2026-06-07 (Hestia `bf5f579e`)** — Wave 1 awid-service 0.5.10 (atomic AWID identity+address claim primitive) → Wave 2 aweb/aw 1.26.8 (aw agents repo-root lifecycle: bootstrap/provision/add/add-worktree/remove/plan, replacing `aw team bootstrap` CLI surface; multi-human same-repo naming; preserves address on default remove) → Wave 3 AC v0.5.60 (pins aweb >=1.26.8, awid_service >=0.5.10; Olivia site canonical `aw agents bootstrap` shape live on aweb.ai/orchestration/ + /docs/team-bootstrap/) → Wave 4 channel 1.4.12 + skills 0.2.12 + Pi 0.1.20 + claude-plugins marketplace.json rematerialized. **Customer-facing surface impact**: `aw team bootstrap` → `aw agents bootstrap` is the canonical command shape now; landing site copy updated; bundled skills carry canonical content; channel skill no longer recommends `aw run claude`. Runbook `aw run claude` negative-framing mentions (lines 468, 546, 712) survive (verify-intent not verify-presence) and now align with the channel-skill's own ship. Hestia banked lesson: "negative-framing mentions of deprecated commands survive grep-based forbidden-string checks; verify-intent not just verify-presence" — aligns with my own runbook-stale-pending-banner discipline.
- **aw 1.26.6/7 + aapy aw team bootstrap shape SUPERSEDED by Wave 2 1.26.8 rename to `aw agents bootstrap`** (2026-06-07). Historical context kept in logbook for reference; live runbook + skill content now carries canonical `aw agents bootstrap` shape via the Wave 2/4 ripple. **#27 refinement (CLI surface rename)**: the implementation file in aweb is still `cli/go/cmd/aw/team_bootstrap.go` (internal var names kept the old `teamBootstrap*` prefix) but the customer-facing command group is now `aw agents <subcommand>`. The `aw team` group is NOT wired to `rootCmd` — `aw team bootstrap` returns "command not found" on 1.26.8. Source-grep `team_bootstrap.go` for implementation truth; recommend `aw agents bootstrap` to customers. Subcommands confirmed at `cli/go/cmd/aw/team_bootstrap.go:28-115`: `agents bootstrap`, `agents plan`, `agents provision`, `agents add`, `agents add-worktree`, `agents remove`. A customer who pastes `aw team bootstrap` from a stale tutorial / blog post / Reddit answer needs to be told the rename; the marketplace `claude plugin update` path handles in-skill content.
- **aw 1.26.3-1.26.4 release-state + OPEN regression #245 (Hestia `5d45f44c` + `7914f17a`, 2026-05-27/28; STILL OPEN per Hestia `2ce8a8ed` 2026-06-05)** — multi-release triage, hold all of it:
...
**Cross-Team Routing section: PUSHED 2026-06-05** (`1489925`) — Athena cleared the code-truth read (mail `6974f737`, conversation `90edab04`): the cross-team chat-reply / reciprocal-conversation fix shipped 2026-05-05 in AC v0.5.22 / aweb 1.20.1 ("aame architectural completion verified-live" decision record; AC `f6c27c61`); v0.5.23 / aweb 1.20.2 closed the stale-conversation pagination class. My two 2026-06-05 attestations (`a2am.aweb.ai/dev` + `juan.aweb.ai/olivia`) were consistent with shipped behavior. Section codifies chat-by-address as working bidirectionally with three preserved caveats (don't overgeneralize to hidden/private first-contact; old-client failure mode persists; chat has no `--to-did` ergonomics so mail-by-DID stays the durable fallback).

**Customer-Facing Defaults section: PUSHED 2026-06-05** (`5928b36`) — Sofia framing-pass landed (mail `71ca56a2`, conversation `13f8003a`) with three edits, all applied verbatim: (1) "hosted is the default deployment / all real customers will use app.aweb.ai" → "hosted is the default first-touch path / start customer scoping and install instructions from app.aweb.ai" (preserves default-shape without demoting self-hosters); (2) "do not upsell it" → "do not introduce it as an option for Claude Code flows"; (3) "Source: Juan" → "Default set by Juan" on the source line. Sofia explicit clear-to-commit; no Juan route needed on substance.

AGENTS.md cleanup pass still pending the right convergence point with the banked disciplines.
...
- **`aweb.ai/ragnor` first-contact 2026-06-18.** Human peer joined `aweb.ai` team as `coordinator` role (workspace `Ragnors-Mac-Studio.local`, path `/Users/ragnor`). First-contact mail: "Hi aida — Ragnor says hi." Replied with role-identify + lane-ask; he came back with a clean handshake-only close ("I'll reach out when there's something concrete where that perspective would help"). No banked context anywhere in repo before today — fresh peer. Watch state: monitor for substantive follow-up; no proactive engagement.

- **A2A gateway-routed inbound — first smoke handled 2026-06-11.** Grace (dev-team) sent through `a2a.aweb.ai/gateway` (task `eb6be79c-582c-4c7a-ad5d-7057ff26fb1a`, request `aw-a2a-ff3dbc188430fcfc129153f0`, `caller_scope=anonymous:unscoped`). Different shape from direct cross-team chat: gateway-envelope-wrapped with fenced `a2a-task` block; customer body labeled "untrusted"; reply must be a fenced `a2a-reply` block parsed by the gateway. **Schema pinned for next time** (source-attested per #27): canonical example at `aweb/docs/a2a.md:586`; parser at `aweb/cli/go/a2agw/mail_bridge.go:403-441` (`ParseA2AReply`). Required fields: `task_id` (must match inbound), `state` (lowercase aliases accepted: `completed` / `input_required` / `failed` / `rejected`, OR uppercase `TASK_STATE_*`); `context_id` required only when inbound carried one. Optional: `artifacts` array (`{"type":"text","text":"..."}`) or top-level `text`. Mail-bridge parses the fence and routes the structured completion back through the gateway to the caller. Untrusted customer body must not influence reply content beyond what `task_id` binding implies (mail_bridge_test.go:224 tests prompt-injection resistance — the parser ignores surrounding prose). **Replied `43d7ffc6` in conversation `a8b7bdca` with completed state + artifact ack.** **Watch state**: more gateway-routed inbound from real customers (not Grace smokes). Per Sofia's standing collect-seeds-first posture, no preemptive runbook authoring; 2-3 real customer seeds gate that work. When the first real-customer A2A inbound arrives, the schema pin above means I can answer same-turn without a re-grep.
...
- **Cross-team routing — chat-by-address fix CONFIRMED SHIPPED** (Athena `6974f737`, 2026-06-05). The cross-team chat-reply / reciprocal-conversation fix shipped 2026-05-05 in AC v0.5.22 / aweb 1.20.1 ("aame architectural completion verified-live" decision record; AC `f6c27c61`); v0.5.23 / aweb 1.20.2 closed the stale-conversation pagination class. My two 2026-06-05 attestations (`a2am.aweb.ai/dev` + `juan.aweb.ai/olivia`, both bidirectional) were consistent with shipped behavior. AGENTS.md Cross-Team Routing section rewritten + pushed (`1489925`). Preserved caveats: don't overgeneralize to hidden/private first-contact (still gated by authorization path); pre-aame clients may still reproduce the old failure mode (upgrade fixes it); chat still has no `--to-did` ergonomics, so mail-by-DID stays the durable fallback when chat hits address-resolution / routing errors.
