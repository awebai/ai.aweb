...
- client packages (npm, as of 2026-06-05 post-1.26.7 patch): `@awebai/claude-channel` 1.4.11, `@awebai/pi` 0.1.17, `@awebai/claude-skills` 0.2.11, `@awebai/aw` **1.26.7** (was 1.26.6 same day). aw 1.26.6 + 1.26.7 are CLI-only (no server-v tag, PyPI aweb stays 1.26.5, no AC, no awid). 1.26.7 adds path-traversal sanitization on `--agents-dir` (e.g., `../escape` → `agents/worktrees/escape`, regression test pinned) + help-text refresh on `--template-cache-dir`; defensive hardening, no customer-facing behavior change for non-malicious aliases. claude-skills 0.2.11 + Pi 0.1.17 ship the aapy in-repo bootstrap ripple (aweb-bootstrap SKILL.md updated for `--agents-dir`/in-repo convention; byte-identical content with aweb `b78fc79`:skills/). Marketplace pin bumped to aweb-skills 0.2.11. Runbook install commands are version-agnostic (marketplace alias / `@latest`) so they don't pin to these.
