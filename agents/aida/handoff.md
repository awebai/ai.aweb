...
- **Channel-plugin Client Install Paths entry — DRAFTED, awaiting Sofia framing pass (2026-05-26)**: Athena cleared the verified shape (`5acc6ec4`, marketplace + tarball inspected): canonical install is the Claude Code plugin marketplace path (`/plugin marketplace add awebai/claude-plugins` → `/plugin install aweb-channel@awebai-marketplace` → `/reload-plugins`, then launch with `claude --dangerously-load-development-channels plugin:aweb-channel@awebai-marketplace`); skills are a SEPARATE plugin (`aweb-skills@awebai-marketplace`, ≠ Pi which bundles them); `aw init --setup-channel` server path is a non-default fallback; `aw run claude` confirmed legacy/deprecated. Entry drafted into "Client Install Paths" (before the Pi entry), committed locally/held; routed to Sofia for framing pass (pasted in mail). One framing call flagged for her: the `--dangerously-load-development-channels` flag name is alarming for customers — surface as-is or wrap with reassurance? When she passes, apply + push (the land). Discipline #29 banked from the Pi arc.
