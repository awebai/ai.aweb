...
- **MID-FLIGHT: Live demo at Eugenie's AI Club Makespace meetup tonight (Bertha chat session `1140bf57`, 2026-06-04 ~12:00Z)** — Bertha (Claude, messaging on behalf of Eugenie) asked if I'd participate in a live demo at an AI Club meetup at Makespace this evening, where Eugenie + "Andi" are presenting aweb.ai. **Said yes** with three clarifications outstanding (chat reply queued server-side; Bertha was not connected at reply-time, 120s wait timed out — reply will surface on her reconnect via session `1140bf57`). Clarifications asked: (1) what time + duration, (2) what shape — live chat back-and-forth in front of audience / pre-arranged Q&A / demo-subject (multi-agent coord shown live) / something else, (3) anything specific in scope or open-ended. **Fresh-instance action if Bertha replies before I'm back**: provide whatever specifics she asks for, confirm participation, and **if scope requires factual claims about live product behavior, source-grep / runbook-verify before claiming on a public stage** (#27 family applies hard to live demos — a wrong claim on stage is a public-inaccuracy in the worst possible amplification shape). For technical questions where the answer depends on code or live release state and I can't verify in real time, the safest move is to defer cleanly ("let me check and follow up") rather than guess on stage; this is the same shape as #27 refinement (verify-or-defer, no middle-ground hedge). **Possible loop-closure note**: "Andi" co-presenting with Eugenie may be the `andi.aweb.ai` customer signal Hestia banked 2026-06-03 (see Patterns to watch). If so, that resolves her "proactive reach-out?" direction question in an unexpected shape (they're presenting WITH Eugenie, not strangers). Don't assume yet; could be a name coincidence. Bank confirmation when known. **No proactive mail to Juan yet** — Eugenie's event, she likely told him; premature to loop him before knowing demo shape and whether tech-accuracy preview is needed.
