...
- client packages (npm, as of 2026-05-27): `@awebai/claude-channel` 1.4.11, `@awebai/pi` 0.1.16, `@awebai/claude-skills` 0.2.10, `@awebai/aw` **1.26.3** (was 1.26.1). Runbook install commands are version-agnostic (marketplace alias / `@latest`) so they don't pin to these.
- **aw 1.26.3 — workspace cleanup regression fix + new `aw workspace delete` (Hestia `5d45f44c`, 2026-05-27)**: `aw workspace status` now correctly cleans up gone LOCAL identities (1.26.2 left them as `unknown_lifetime_no_cleanup`). **Triage**: a customer on 1.26.2 reporting gone-local-identities-not-clearing → recommend upgrade to 1.26.3 (the recommended customer upgrade from 1.26.2; customer-reported regression, now fixed). New command `aw workspace delete <workspace-id-or-alias>` deletes a local workspace + local identity; server protects global/non-local (won't delete server-anchored). It's DESTRUCTIVE → if a customer asks how to delete a workspace/local identity, route the safe specifics through Athena (destructive-op discipline) rather than advising from this notice; not authoring a runbook entry yet (collect-seeds-first, no asks). My existing `aw workspace status` / `add-worktree` runbook references are unaffected by this change (#27 grep-checked).
