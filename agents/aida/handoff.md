...
- **Channel-plugin Client Install Paths entry — IN FLIGHT (2026-05-26)**: Sofia greenlit (`6d7a5794`) a parallel Claude Code channel-plugin entry mirroring the Pi entry's shape (install cmd / one-line what / one-line when / "one client among several"). Waiting on Athena (`a73f5b37`) for the verified install command shape (#27 — don't infer it). Sequence per Sofia: Athena clears command shape → I draft → route to Sofia for framing pass → land. Discipline #29 banked from this arc (verify-assumed-structure-before-acting).
