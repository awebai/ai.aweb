...
- client packages (npm, as of 2026-06-05): `@awebai/claude-channel` 1.4.11, `@awebai/pi` 0.1.16, `@awebai/claude-skills` 0.2.10, `@awebai/aw` **1.26.6** (was 1.26.4). 1.26.6 is CLI-only (no server-v tag, PyPI aweb stays 1.26.5, no AC, no awid). Runbook install commands are version-agnostic (marketplace alias / `@latest`) so they don't pin to these.
- **aw 1.26.6 release (Hestia `2ce8a8ed`, 2026-06-05)** — aapy in-repo bootstrap. New `aw team bootstrap --agents-dir <name>` flag: project-local directory created inside current git repo instead of the legacy home-root / work-directory pattern. Mixed-flag rejections enforced (`--agents-dir` cannot combine with `--work-directory` / `--work-repo-url`; `--home-root` cannot combine with in-repo bootstrap). aw CLI-only release (no server-v tag, PyPI aweb stays 1.26.5, no AC, no awid). Target SHA `b78fc79`; canonical gate green (415 e2e tests passed). #253 closed. **Customer-facing ripple**: Olivia is coordinating skills/Pi ripple per Grace's release-shape #3 — `aweb-bootstrap` SKILL.md text must match the new `--agents-dir` convention (canonical updates already in aweb `b78fc79`; codex plugin mirror synced). My runbook only references `aweb-bootstrap` as a skill name (line 508, 578); CLI-command-shape recommendations not present. No support-surface update needed; the skill owns the customer-facing bootstrap text. **Standing watch**: if a customer asks about `aw team bootstrap` before the skill ripple lands and the answer must reflect `--agents-dir`, source-grep `cli/go/cmd/aw/team_bootstrap.go` for current flag shape before recommending (#27 family).
