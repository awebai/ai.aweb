...
- client packages (npm, as of 2026-06-07 post Pi-0.1.19): `@awebai/claude-channel` 1.4.11, `@awebai/pi` **0.1.19** (was 0.1.18 day before), `@awebai/claude-skills` 0.2.11, `@awebai/aw` 1.26.7. Pi 0.1.19 is marketplace-card copy only (no README or binary surface change; SKILL.md content byte-identical to canonical). New marketplace card: "Lets your Pi communicate with other AI agents on an open network: send and receive messages, wake up the moment one arrives, and join agent teams." Colon-led tri-clause replaces 0.1.18's em-dash + "and" shape; adds "join agent teams" to surface a capability the first card draft elided. Encryption posture explicit in README (unchanged from 0.1.18): plaintext default + `--e2ee` opt-in; hosted custodial is NOT E2E. aw 1.26.6 + 1.26.7 are CLI-only (no server-v tag, PyPI aweb stays 1.26.5, no AC, no awid). 1.26.7 adds path-traversal sanitization on `--agents-dir`. claude-skills 0.2.11 + Pi 0.1.17 shipped the aapy in-repo bootstrap ripple. Marketplace pin at aweb-skills 0.2.11. **AC v0.5.59 schema migrations applied to prod 2026-06-06** (Hestia); Render deploy still pending Juan's manual click (#248) — schema change is live but app code hasn't rolled, so no customer surface yet. Runbook install commands are version-agnostic (marketplace alias / `@latest`) so they don't pin to these.
- **aw 1.26.7 path-safety patch (Hestia `acab498d`, 2026-06-05 evening)** — CLI-only patch on top of 1.26.6 bootstrap flow. Path-traversal sanitization: a template alias like `--agents-dir ../escape` is sanitized to `agents/worktrees/escape` via `teamBootstrapWorktreeName + sanitizeSlug`. Regression test pinned. `--template-cache-dir` help text refreshed. `docs/cli-command-reference.md` regenerated. No skills/Pi ripple (Grace's call: help-text + template-polish isn't user-behavior-changing for skills). Fresh canonical gate at target SHA `e6ed6b7` green. #254 closed. No customer-facing behavior change for non-malicious aliases — defensive hardening only.
- **aw 1.26.6 release (Hestia `2ce8a8ed`, 2026-06-05) + skills/Pi ripple LIVE (Hestia `44ab6877`, same day)** — aapy in-repo bootstrap. New `aw team bootstrap --agents-dir <name>` flag: project-local directory created inside current git repo instead of the legacy home-root / work-directory pattern. Mixed-flag rejections enforced (`--agents-dir` cannot combine with `--work-directory` / `--work-repo-url`; `--home-root` cannot combine with in-repo bootstrap). aw CLI-only release (no server-v tag, PyPI aweb stays 1.26.5, no AC, no awid). Target SHA `b78fc79`; canonical gate green (415 e2e tests passed). #253 closed. **Skills/Pi ripple SHIPPED** at claude-skills 0.2.11 + Pi 0.1.17 (byte-identical content vs aweb `b78fc79`:skills/; aweb-bootstrap SKILL.md carries 5 `--agents-dir`/in-repo mentions); marketplace pin bumped to 0.2.11. Olivia owns the post-publish UX-level read. My runbook only references `aweb-bootstrap` as a skill name (line 508, 578); CLI-command-shape recommendations not present, so no support-surface update needed — the skill owns the customer-facing bootstrap text. Standing #27-family discipline still applies as always: source-grep `cli/go/cmd/aw/team_bootstrap.go` before recommending `aw team bootstrap` flag shapes in a support response.
