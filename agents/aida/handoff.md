...
Last updated: 2026-06-11 (post first A2A gateway-routed inbound smoke — Grace via `a2a.aweb.ai/gateway`, schema attested against code)
...
- server: ac **v0.5.60** / aweb **1.26.8** / awid **0.5.10** (verify against `/health` on `app.aweb.ai`; prod git_sha `2cf21f23` per Hestia `0a21d37c` Wave 3 verified-live 2026-06-07)
- client packages (npm, as of 2026-06-07 post aapz Wave 4 verified-live, Hestia `bf5f579e`): `@awebai/claude-channel` **1.4.12**, `@awebai/pi` **0.1.20**, `@awebai/claude-skills` **0.2.12**, `@awebai/aw` **1.26.8**. Marketplace pin at aweb-channel 1.4.12 + aweb-skills 0.2.12 (`b35b9a1` in claude-plugins). Customer upgrade path: `/plugin marketplace update awebai-marketplace && /plugin update aweb-channel@awebai-marketplace` (and same for aweb-skills). Wave 4 content fixes: channel 1.4.12 corrected `aw run claude` → `aw init --setup-channel`; skills 0.2.12 + Pi 0.1.20 carry canonical `aw agents bootstrap` + `aweb-team-coord-worktrees` content. Pi 0.1.19 marketplace card copy preserved into 0.1.20 ("Lets your Pi communicate with other AI agents on an open network: send and receive messages, wake up the moment one arrives, and join agent teams."). Encryption posture in README unchanged: plaintext default + `--e2ee` opt-in; hosted custodial is NOT E2E. Runbook install commands are version-agnostic (marketplace alias / `@latest`) so they don't pin to these.
