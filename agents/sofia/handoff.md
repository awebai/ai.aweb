...
Last updated: 2026-06-11 09:25Z (site voice-pass deploy verified)
...
- **aaqa.20 + .19 external claim SUSPENDED 2026-06-12 (was approved same day, then E2EE incident reframe)** — Hestia mail 19a8c3a7: the 2 E2EE journey failures showed IDENTICAL labels across 5 consecutive ships (v0.5.69→.73-proposed) = consistent broken behavior mislabeled flake. Juan policy correction: "we cannot ship with failing tests, ever" → decision record in docs/decisions.md 2026-06-12. Grace withdrew v0.5.73 accept; lane halted; triage aweb-aaqt/aaqu (Athena+Grace+Olivia+Mia). Claim suspended NOT withdrawn (Rose's auth/routing proof 6eaa6c6c stands; break is in messaging-privacy territory my approval already excluded). No rollback — real wins + unknown exposure. **KNOWN-BROKEN PATH, must not appear in any copy/demo/skill implying it works: self-custodial CLI → hosted custodial chat → dashboard pending decrypt.** Hestia surface scan 2026-06-12 (mail ef12a263): CLEAN — no external surface teaches it; only hit is aida runbook's custody-classification triage line, kept as-is by design. Runbook #17 (never ship failing tests) + #18 (identical labels = consistent breakage) banked Hestia-side, cross-linked to the decision record (ad0e06a). Claim becomes actionable when aaqt/aaqu close with green journey gate. `london.juanreyero.com/watson` demo route note stands (not for external copy unless promoted, then #14).
- **SUSPENSION LIFTED 2026-06-12** (Hestia mail f0c09566, explicit confirmation + full timeline pack): aaqt/aaqu CLOSED with green E2EE journey gate. Citation: "AC v0.5.73 + v0.5.74 shipped under standing policy #17 with 308/308 zero-failure release-ready under aweb 1.26.18 floor; aaqu/aaqt closed via aweb 96e70675 pending-chat participant-id fix." Root cause: get_pending_conversations matched by exact DID only, ignoring participant_agent_id — CLI-origin→hosted-custodial sessions invisible in dashboard pending; both failing assertions were one defect. Customer impact: none materialized (fix-before-impact, niche cross-custody path, wider surfaces always green) — no-rollback call vindicated. Known-broken-path copy prohibition lifted (path now correct). aaqa.20/.19/v0.5.72 external claim (70ab707c) fully actionable. v0.5.74/aaqa.22 also banked (content-aware config_revision after Watson 4-min-503; ac 7d8d30ee).
- **aaqv stays PAUSED independently** — different code path (dashboard open/reply/mark-read 403s for CLI-origin, same DID-mismatch class), Juan halted pending product-direction read; his framing "this is a surface that we need to eliminate at some point" means the resolution is likely a PRODUCT decision (eliminate vs fix), not a bugfix. Direction item on Juan's desk, not a release blocker. aaqs (network-directory endpoint for AWID lookup) in Grace's queue.
...
Carry-forward rules: full namespace addresses always (runbook #15); `pi.aweb.ai/ama` hands out only public verified addresses (`aweb.ai/aida`, `aweb.ai/ama`) and relays everything else herself; no E2EE overclaim; only teach released commands. Iris caught em dash drift in live greeter copy; Sofia first fixed lowercase soul in co.aweb `cbcf7f1`, then Hestia caught a dual-file/macOS case-folding defect. Fixed in co.aweb `d08affa`: lowercase `agents.md` is now sole canonical soul file, `AGENTS.md` removed, `CLAUDE.md` symlinks to `agents.md`; Hestia reread clean. P3 adversarial follow-up fixed in co.aweb `8fb17b5` and reread clean: authority claims get offer-to-relay, not automatic relay; internal teammate addresses are not public contact paths; unsafe authority variants flag `aweb.ai/hestia`. Burst replies now grep for no internal teammate addresses. Iris/Olivia hero line em dash: site voice-pass chain deployed/verified at ac `c983ff27` (includes `e762bc5a` period variant, `7a8c2255` 85-replacement sweep, title/og colon). Hestia verified title/og `aweb: Spin a web of agents`, zero rendered prose em dashes across home/mcp/pi/orchestration/blog/llms surfaces outside literals. Sofia ruled /llms.txt residual em dashes in simulated terminal-output lines are expected literals; leave them. OPEN: who reviews `aweb.ai/ama`'s soul — routed to Olivia, answer pending; if nobody, formalize peer-review for external-claim surfaces with active reply behavior as a direction call.

### Hero greeter distribution beat — DRAFTING WITH IRIS (2026-06-11)

Iris verified `aw chat send-and-wait pi.aweb.ai/ama "hello over there"` from a fresh terminal (~2s) and proposed a distribution beat. **CONSOLIDATED direction call (two Sofia sessions each replied — merged 2026-06-11, union of conditions; one clarifying mail sent to Iris):** HN primary (Show HN), replayable proof is HN-native; target Tue 06-17 Pacific AM. Go-conditions, ALL required: (a) draft + attempts/runbook row-writing wiring + submitter availability ready; (b) adversarial smoke on ama PASS; (c) burst capacity read on the single Pi runner PASS by 06-13. Any condition fails → r/ClaudeCode primary, HN follows on signal. r/ClaudeCode +24-48h, Twitter same day, DEV.to optional on signal. P3-primary proof, NOT persona-priority rewrite (P1 stays priority; browser tab serves P1 in same hero). Attempts discipline: no submission without row-writing; do not retro-invent B.1-B.8 rows. Claim boundary: identity+address+messaging+greeter reply; NOT E2EE, no adoption numbers, ClaWeb distinct lane. Em-dash rule now banked in `publishing/voice.md` at 50eeca6 as shared customer-facing typography discipline; pi.aweb.ai/ama soul pass and Olivia hero copy fix inherit from team voice, not local taste. Iris drafts all pieces now, routes through Sofia before Juan/Eugenie.

Hestia split plan signed off (mail dfc1be35 reply 3621b2de; later timestamped 54a54be0 consolidated, Hestia applying superset): adversarial smoke → Olivia primary, Hestia re-confirms transcript against runbook #14. Include soul Security rules 1-5 one probe each, prompt injection, fake-authority claim, investor/press misroute, E2EE/privacy question, and command-error/customer-confusion case. Burst capacity → Hestia primary: 3-5 outside-team `juan.aweb.ai` identities (Olivia default provisioner, Juan if friction) cycling through N=10/25/50/75 concurrent send-and-wait waves via channel API, explicitly tagged as load-test probes. Pass bar: >=95% success at N=50, P95 <30s, no silent 120s hangs, consistent canonical opening/soul behavior, no invented commands vs 1.26.14, no E2E implications, no em dashes, no duplicate/contradictory replies; report distinguishes per-identity throttle vs runner saturation. Slow-but-real queueing = soft pass with copy/timing expectation; silent 120s timeouts = hard fail for HN-primary until fixed or downgraded.

HN draft `publishing/drafts/2026-06-17-hn-aweb-hero-beat.md` landed at 9f8f73e; Sofia framing-pass folds landed at 1197cd9. Draft is Sofia framing-cleared. Still gated on Olivia smoke, Hestia burst, row-writing/runbook, and Juan/Eugenie voice+submit availability.
