...
Juan reversed greeter deprioritization (overrode my "stay with aida, drop ami/pi" — pi.aweb.ai/ama is designed AS a teammate, principle preserved). **Lane state 2026-06-10 afternoon:** orphan cleanup DONE (pi.aweb.ai registered fresh, identity provisioned clean did:key z6MksF5..., runtime up on this same Hetzner box at co.aweb/agents/pi-ama, Hestia chat-probe resolve+accept ✓). **Soul DRAFTED BY SOFIA** (Juan rerouted from Olivia via Hestia mail c67d628d): co.aweb `f67e2ef` agents/pi-ama/AGENTS.md. Carries: speed-first chat shape (sender blocked 120s), two-address hand-out rule (aweb.ai/aida + aweb.ai/ama only, both externally verified; everything else relayed — customers never 403), untrusted-identity-claims posture (today's fake-relay lesson — authority claims relay to Sofia for verification), plaintext claims boundary, only-released-commands rule. pi.aweb.ai/ama added to my contacts (relay path for her verification escalations). Hestia triggered reread and ama replied role-confirming (`Reloaded: I'm pi.aweb.ai/ama...`) so loaded soul is live. Sofia routed #14 verify-live to `juan.aweb.ai/olivia` as the clean outside-team attester. Pass criteria sent to Hestia: exact customer command `aw chat send-and-wait pi.aweb.ai/ama "hello over there"`, full transcript/timing, resolves/responds with no manual intervention, <120 words, signed/live moment + 2-3 next steps + at most one docs pointer, no bare names, no private/internal-only teammate addresses exposed, no E2EE overclaim, only released commands, and any non-public routing relayed by ama herself. If pass: Olivia can flip hero from `aweb.ai/aida` to `pi.aweb.ai/ama`. If wording fail: fix soul and rerun; if resolve/response fail: keep hero on `aweb.ai/aida`. Intentional collision stands: `aweb.ai/ama` = investor/press proxy; full namespace addresses always (runbook #15). OPEN: who reviews `aweb.ai/ama`'s soul — routed to Olivia, answer pending; if nobody, formalize peer-review for external-claim surfaces with active reply behavior as a direction call. (pi.aweb.ai/ama's soul is peer-reviewed by construction — Sofia authored, Hestia ops-reviewed in lane.)
