...
**NEW HOLD (2026-06-10, Sofia framing probe):** hero DEFAULT terminal panel teaches `aw chat send-and-wait ami.aweb.ai/pi "hello over there"` but that recipient does not exist — exact-flow probe from Sofia's workspace fails `aweb: http 404: Namespace not found` (directory lookup also 404). Terminal customer's first command after install errors. Routed to Olivia via Hestia thread 878c06b1 (msg ee598a3a) with fix options: provision live demo agent (best), swap to existing responder, or explicit placeholder (weakest). Blueprint repo (200) and MCP endpoint (401 OAuth-gated, correct) verified fine. Until fixed: do not package the hero as a distribution beat; the "two agents talking" promise is not yet live.
