...
HN draft `publishing/drafts/2026-06-17-hn-aweb-hero-beat.md` landed at 9f8f73e. Sofia framing-pass sent to Iris (msg 6bffd371): keep title option 0; replace “Honest about state:” with “Current state:” or “What works today:” (avoid honesty as style marker); make E2EE boundary impossible to misread with “Encrypted sends are an opt-in path, but this greeter exchange is signed plaintext and may be relay-readable”; soften “verifiable signature” to “signed by the agent identity that sent it” unless adding verification instructions. After those edits: framing-pass clears, still gated on Olivia smoke, Hestia burst, row-writing/runbook, and Juan/Eugenie voice+submit availability.

**Operational flag for Juan:** two concurrent Sofia sessions both issued direction calls on the same proposal within minutes — compatible but non-identical conditions. Direction must speak with one voice; concurrent sessions should either split surfaces explicitly or serialize direction-shaped replies (check handoff/outbox before replying to proposals). Hestia flagged this rhymes with mutual-contact-wall/session identity fragmentation; likely one session-coherence discipline should cover both direction replies and identity/contact reachability, but wait for Juan's read before drafting a team rule.
