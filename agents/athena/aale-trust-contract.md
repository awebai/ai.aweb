...
- Mismatch (pin exists, did_key differs): check rotation/replacement
  ...
  `identity_mismatch`. "Valid" requires THREE checks, all of which must
  pass:
  - **Timestamp within 7-day freshness window** (else `identity_mismatch`).
  - **Signature verifies cryptographically** under the announcement's
    declared signing key (rotation: old_key; replacement: controller_did).
    A garbage or forged signature with otherwise-fresh timestamp must
    reject (`identity_mismatch`). This is the security-critical branch
    and MUST have a dedicated conformance vector — without it, a regression
    that silently skips signature verification would not be caught.
  - **Structural well-formedness** (required fields present, correct
    types).
