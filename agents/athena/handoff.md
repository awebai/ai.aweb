# Athena Handoff
Last updated: 2026-05-21 23:27 GMT
...
- Last fully validated aapj heads remain aweb `fa1041c` + AC `9104ffc2`, green in Hestia's sibling-source validation chain. Current pre-release blocker heads have moved: aweb main is `f31ffcb` (aapk.1 + aapl.1); AC main is `93fb5d3e` (aapk.2 + aapk.3 Phase A). No tag/deploy/release.
...
- New pre-release blocker `aweb-aapk`: Juan clarified onboarding auto-provision blocks release. Direction: drop the post-signup "create your first team" gate; register auto-provisions a personal team from username and lands on that dashboard; BYOT/explicit team creation remains later. `.1` closed at aweb `b7e2192`. `.2` backend is on AC main `737ecf89` and Athena re-review passed after blockers were patched: availability checks AC user + AWID namespace + `teams/default` and fails closed on registry errors/missing client; `/register` provisions `default:<username>.<base>`; `/auth/me` idempotently repairs missing, blank/partial, and old noncanonical `<username>:<username>.<base>` rows without deleting user/org state. `.3` Phase A landed on AC main `93fb5d3e` from Olivia's Juan-walked branch: HomeRedirect drops the normal `/onboarding` gate, connect UI uses self-custodial/custodial vocabulary, role rows hide when no playbooks, and Makefile editable sibling-overlay dev path has release guardrail (`release-verify-model` fails on editable `aweb`/`awid-service`; `dev-backend` warns). Athena landing validation: diff-check, overlay script py_compile, clean/no-venv pass, synthetic editable failure, focused frontend 15 passed; Olivia reported full frontend 195/195. Mia still owns final `.3`: setup-progress checklist/log using `default_team_owner_username`, `default_team_namespace_domain`, `default_team_dashboard_team_id`, `default_team_dashboard_url`, plus remaining copy/help text.
- New pre-release blocker `aweb-aapl`: Juan wants global agents to opt into contact-or-team inbound without going fully public. `.1` is closed at aweb `f31ffcb`: explicit `inbound_mode=open|contacts_or_teammates|contacts_only`; `contacts_only` stays strict; same-team allowance uses verified team-certificate auth only; mail/chat regressions and docs updated. `.2` is with Mia after aapk.3 for AC mirror/UI; backend/provisioning must default omitted global identities to `contacts_or_teammates`, not just frontend forms. `.3` is Grace review after AC mirror.
