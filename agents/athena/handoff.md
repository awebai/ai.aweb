...
- Last fully validated aapj heads remain aweb `fa1041c` + AC `9104ffc2`, green in Hestia's sibling-source validation chain. Current pre-release blocker heads have moved: aweb main includes aapk.1 at `b7e2192`; AC main is `737ecf89` with aapk.2 backend. No tag/deploy/release.
...
- New pre-release blocker `aweb-aapk`: Juan clarified onboarding auto-provision blocks release. Direction: drop the post-signup "create your first team" gate; register auto-provisions a personal team from username and lands on that dashboard; BYOT/explicit team creation remains later. `.1` closed at aweb `b7e2192`. `.2` backend is now on AC main `737ecf89` and Athena re-review passed after blockers were patched: availability checks AC user + AWID namespace + `teams/default` and fails closed on registry errors/missing client; `/register` provisions `default:<username>.<base>`; `/auth/me` idempotently repairs missing, blank/partial, and old noncanonical `<username>:<username>.<base>` rows without deleting user/org state. Mia reported sibling-source backend sweep `1446 passed, 15 deselected`; Athena routed backend release-gate slice to Grace. `.3` frontend remains with Mia: remove signup team gate, route to `default_team_dashboard_url`, show setup-progress checklist using `default_team_owner_username`, `default_team_namespace_domain`, `default_team_dashboard_team_id`, `default_team_dashboard_url`, then offer first self-custodial CLI or custodial MCP path. Olivia pushed `origin/olivia-aapk-3` as Juan-walked product input only; Mia owns implementation. Important guardrail: if the Makefile editable sibling-overlay change is folded, it must include release/dev warnings so local editable aweb/awid does not masquerade as shippable pinned-lock validation.
