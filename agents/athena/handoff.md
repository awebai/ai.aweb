...
- Juan asked to keep idle agents applied. Athena created/routed `aweb-aapj.9` to Peter: read-only file-by-file rewrite map for AC `.3` Phase B 2/2. Athena created/routed `aweb-aapj.10` to Dave: final `.5` grep-gate dry-run prep. Mia ACKed and is actively taking `.3` Phase B 2/2; she has pushed partials through AC `5338b304` and continues. Peter sent raw `.9` notes; Athena forwarded them to Mia, but kept Peter focused on `.12` unless Mia later asks for a concrete disjoint file group. Important correction after Juan pushback: do NOT keep derived `legacy_lifetime` from canonical `identity_scope`; support/admin/audit may expose old fields only for actual persisted old state or external legacy input, not derived compatibility residue. This was documented in `docs/invariants.md` and sent to Mia/Dave. Dave’s `.10` dry-run surfaced two more pre-`.5` blockers: `aweb-aapj.11` to Grace for remaining public CLI/SOT docs cleanup and `aweb-aapj.12` to Peter for AWID team-certificate API/storage canonical `identity_scope` cleanup. `.11` is closed at aweb `5f4dc04`; `.12` is closed at aweb `bdc39e4`. Dave keeps `.10` open/standby until final refs. Grace created `aweb-aapj.13` for AWCO/BYOIDT team-certified signed request mode; initial implementation `7ce0f39` plus evidence follow-up `2245b67` are on aweb main but Athena did not close it. Remaining blockers sent to Grace: add verifier cases for missing team certificate and revoked certificate. Need confirm whether `.13` gates immediate release or is parallel P0 after cleanup gate.
