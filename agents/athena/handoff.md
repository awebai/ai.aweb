# Athena Handoff
Last updated: 2026-05-23 17:23 GMT
...
- **New P0 policy epic:** Grace created `aweb-aapq` to restore restricted team-scoped delivery for global agents after Juan stepped back from exact-contacts-only. `aweb-aapq.1` contract is now frozen: aweb runtime `inbound_mode` on `aw inbound-mode`, canonical enum `open` / `team_and_contacts` / `contacts_only`; no AWID reachability surface; no sender-declared team trust; `team_and_contacts` means exact active contact OR verified same-team delivery from trusted server/auth context; multi-team globals evaluate overlap across active sender/recipient identity memberships; `contacts_only` stays exact-contact strict; continuations re-run the same predicate and cannot bypass. Grace ACKed and is implementing against that test bar.
