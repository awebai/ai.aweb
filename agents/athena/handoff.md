...
- Juan asked to keep idle agents applied. Athena created/routed `aweb-aapj.9` to Peter: read-only file-by-file rewrite map for AC `.3` Phase B 2/2. Athena created/routed `aweb-aapj.10` to Dave: final `.5` grep-gate dry-run prep. Mia ACKed and is actively taking `.3` Phase B 2/2; she pushed partial `4ea825c8` (dashboard + immediate_connect) and estimates 4-6h total. Peter sent raw `.9` notes; Athena forwarded them to Mia, but kept Peter focused on `.12` unless Mia later asks for a concrete disjoint file group. Dave’s `.10` dry-run surfaced two more pre-`.5` blockers: `aweb-aapj.11` to Grace for remaining public CLI/SOT docs cleanup and `aweb-aapj.12` to Peter for AWID team-certificate API/storage canonical `identity_scope` cleanup. `.11` is now closed at aweb `5f4dc04` after Athena validation; `.12` remains open. Dave keeps `.10` open/standby until final refs.
