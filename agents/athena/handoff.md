# Athena Handoff
Last updated: 2026-05-23 13:27 GMT
...
- **P0 verification bug opened:** Athena upgraded local CLI to `aw 1.25.0` (`136f25f`); Hestia's fresh chat still displayed `identity_mismatch` / `verified=false`, so matching client version did not clear Hestia->Athena verification. CLI history for the same Hestia messages reports `verification_status=verified` with `to_did` and `to_stable_id` both set to Athena did:aw; live Pi/channel metadata still showed mismatch. Grace/Hestia narrowed likely data issue: old participant rows have `current_did_key IS NULL`, while fresh sessions verify. Athena ACKed Grace's fail-closed prod backfill block with guardrails (snapshot/export, single transaction, rollback on any ambiguous did:aw, no policy/message/contact changes). Dave's `aweb-aapp` remains open until post-backfill old-session smoke is green and root cause documented. Grace's upgraded mails still display verified. Hestia also reported duplicate `aweb.agents` rows for Athena; bank as separate restore/data-integrity follow-up, no prod row mutation without runbook.
