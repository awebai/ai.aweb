# Athena Handoff
Last updated: 2026-05-21 10:52 GMT
...
- Juan asked to keep idle agents applied. Athena created/routed `aweb-aapj.9` to Peter: read-only file-by-file rewrite map for AC `.3` Phase B 2/2. Athena created/routed `aweb-aapj.10` to Dave: final `.5` grep-gate dry-run prep. Mia ACKed and is actively taking `.3` Phase B 2/2; she has pushed partials through AC `78eefd02` and continues. Peter sent raw `.9` notes; Athena forwarded them to Mia, but kept Peter focused on `.12` unless Mia later asks for a concrete disjoint file group. Athena decision for Mia: admin/support/audit old fields must move under nested `legacy:{...}` in `.3`, not follow-up; Mia applied this to archive/replace/replacement audit envelopes and was told to rename internal SQL aliases to `legacy_lifetime`. Dave’s `.10` dry-run surfaced two more pre-`.5` blockers: `aweb-aapj.11` to Grace for remaining public CLI/SOT docs cleanup and `aweb-aapj.12` to Peter for AWID team-certificate API/storage canonical `identity_scope` cleanup. `.11` is closed at aweb `5f4dc04`; `.12` is closed at aweb `bdc39e4`. Dave keeps `.10` open/standby until final refs. Grace created `aweb-aapj.13` for AWCO/BYOIDT team-certified signed request mode; initial implementation is on aweb main `7ce0f39` but Athena did not close it. Blockers sent to Grace: add tests for no-`--sign` team-auth, global-workspace team-auth, and verifier rejection fixture/evidence. Need confirm whether `.13` gates immediate release or is parallel P0 after cleanup gate.
