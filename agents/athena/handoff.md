...
- `aweb-aapg` route-level release work is closed as engineering complete, but AC release recovery remains in progress.
  ...
  - Local AC WIP exists and is **not pushed**: `backend/pyproject.toml` set to `awid-service>=0.5.7`, `permanent_addresses.py` removes `reachability=`, `hosted_identity_provisioning.py` accepts/deletes deprecated `address_reachability`; `backend/uv.lock` is not final because PyPI lacks `awid-service==0.5.7` until Hestia publishes.
- `aweb-aaph` is the new P0 product-authority simplification successor epic. Grace documented product authority in aweb `docs/product-authority-sot.md` (`f7133d3`, clarified at `8004485`). Athena agreed and created `aweb-aaph` rather than reopening `aweb-aapg`. Grace briefly created `aweb-aapg.8` with child tasks, then agreed `aweb-aaph` is cleaner; Athena closed the `aweb-aapg.8` tree as superseded and preserved Grace's breakdown as an `aweb-aaph` comment. Grace then created live `aweb-aaph.1`-`.7` children; leave them unassigned until awid-service/AC release recovery is clean. Juan explicitly wants Athena to lead implementation to completion with Dave and Peter, with Grace as reviewer/boundary-checker. Start with `.1` + `.2` as the coupled deletion/blocker slice. Scope: delete dashboard/browser hosted-local identity shape, reject hosted custodial `identity_type=local`, preserve CLI/API-key local self-custodial workspace path, preserve mixed-custody teams through certificates, and test BYOT pending custodial activation fail-closed. `aweb-aaph.6` is broad: OSS aweb docs, AC site/static/docs, hosted app/dashboard copy, agent-facing docs, tutorials, references, support docs, generated/static docs.
