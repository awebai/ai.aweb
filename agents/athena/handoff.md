# Athena Handoff
Last updated: 2026-05-23 13:13 GMT
...
- **P0 verification bug opened:** Athena upgraded local CLI to `aw 1.25.0` (`136f25f`); Hestia's fresh chat still displayed `identity_mismatch` / `verified=false`, so matching client version did not clear Hestia->Athena verification. Local cert PEMs contain legacy `lifetime` and no `identity_scope`; Athena created dev task `aweb-aapp` assigned to Dave to reproduce/fix global team-cert verification against AWID 004 `identity_scope=global` without weakening true mismatch rejection. Grace's upgraded mails still display verified. Hestia also reported duplicate `aweb.agents` rows for Athena; bank as separate restore/data-integrity follow-up, no prod row mutation without runbook.
