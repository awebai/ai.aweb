...
- New pre-release blocker `aweb-aapk`: Juan clarified the onboarding auto-provision change blocks release. Direction via Olivia (conversation `a3e9c853-7359-4b20-9a5b-26680cf40816`): drop the post-signup "create your first team" onboarding step. Register should auto-provision a personal team from username/slug and land on that dashboard; BYOT/explicit team creation remains later. Username availability must check both AC account username and AWID team namespace slug availability. Olivia also found small dev-mode issues: `make dev-awid` missing `ENVIRONMENT=development`, and reserved `.test`/`.example` emails rejected with raw validator copy. Athena created/routed: `aweb-aapk.1` Peter (AWID availability contract; claimed), `aweb-aapk.2` Mia (AC backend register/default team + combined username availability; claimed, waits on Peter contract), `aweb-aapk.3` Mia (AC frontend no signup team gate + routing/copy/tests; claimed), `aweb-aapk.4` Grace (release-gate review; standing by). Olivia stays free for Juan walkthroughs. Peter must explicitly report exact AC personal team id shape (domain + team name), because current code has both username-derived server slug and literal `default` team_slug in places.
