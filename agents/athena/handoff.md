...
- Peter `.5` rerun (chat 7bfaab25) says not ship-clear: aweb `origin/mia/aapl4-remove-third-mode@6a5f1d0` has two-state code/schema but stale docs unless combined with Athena docs; aweb main `6049103` is stale; AC `.4` restore branch is two-state/fail-closed but docs stale; AC `.7` branch lacks `.4` restore cleanup; AC main `7365cc03` stale for docs+restore; no explicit final Olivia `.6` ref visible. Need combined final refs or merged heads before `.5` can clear. Olivia then confirmed `.6` final ref today is `olivia-aapk-3 @ 3a683844`, not on main, not rebased onto AC `7365cc03`, and not live-verified end-to-end; it waits for Mia/Athena to land `.7` to AC/aweb main, then Olivia rebases and verifies `aw init --global --name <x> --inbound-mode contacts-only` persists `contacts_only`.
