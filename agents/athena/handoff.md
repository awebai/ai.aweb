# Athena Handoff
Last updated: 2026-05-22 08:55 GMT
...
- Last fully validated aapj heads remain aweb `fa1041c` + AC `9104ffc2`, green in Hestia's sibling-source validation chain. Current pre-release blocker heads have moved: aweb main is `f31ffcb` (aapk.1 + aapl.1); AC main is `0941ee42` (aapk.2 + aapk.3). No tag/deploy/release.
...
- New pre-release blocker `aweb-aapk`: Juan clarified onboarding auto-provision blocks release. Direction: drop the post-signup "create your first team" gate; register auto-provisions a personal team from username and lands on that dashboard; BYOT/explicit team creation remains later. `.1` closed at aweb `b7e2192`. `.2` backend is on AC main `737ecf89` and Athena re-review passed after blockers were patched: availability checks AC user + AWID namespace + `teams/default` and fails closed on registry errors/missing client; `/register` provisions `default:<username>.<base>`; `/auth/me` idempotently repairs missing, blank/partial, and old noncanonical `<username>:<username>.<base>` rows without deleting user/org state. `.3` setup-progress/UI is closed at AC main `0941ee42`: HomeRedirect drops the normal `/onboarding` gate, connect UI uses self-custodial/custodial vocabulary, role rows hide when no playbooks, Makefile editable sibling-overlay dev path has release guardrail, SetupProgressLog renders canonical `/auth/me` values behind `?welcome=1`, HomeRedirect forwards the flag to the resolved team dashboard, and SetupProgressLog is now a modal with corrected `username.aweb.ai/agentname` copy and explicit Done dismiss. Athena landing validation across `.3`: overlay/script checks and focused frontend tests (15 + 12 + 5 passed); Olivia reported full frontend 195/195 then 207/207. Athena accepted Mia's closure packet and routed full aapk review to Grace.
- New pre-release blocker `aweb-aapl`: Juan wants global agents to opt into contact-or-team inbound without going fully public. `.1` is closed at aweb `f31ffcb`: explicit `inbound_mode=open|contacts_or_teammates|contacts_only`; `contacts_only` stays strict; same-team allowance uses verified team-certificate auth only; mail/chat regressions and docs updated. `.2` is with Mia for AC mirror/UI; Athena approved Mia's plan with corrections: use existing PATCH endpoint (not PUT), include the missed `agent_lifecycle.py` replacement global-agent insert, do not leave new global `awid_member_projection.py` rows default-open, and add package-dashboard AgentDetailPage/API tests. Backend/provisioning must default omitted global identities to `contacts_or_teammates`, not just frontend forms. `.3` is Grace review after AC mirror.
