...
- Peter `.5` rerun (chat 7bfaab25) says not ship-clear: aweb `origin/mia/aapl4-remove-third-mode@6a5f1d0` has two-state code/schema but stale docs unless combined with Athena docs; aweb main `6049103` is stale; AC `.4` restore branch is two-state/fail-closed but docs stale; AC `.7` branch lacks `.4` restore cleanup; AC main `7365cc03` stale for docs+restore; no explicit final Olivia `.6` ref visible. Need combined final refs or merged heads before `.5` can clear. Athena ACKed Peter and asked Olivia for current `.6` ref/status.
