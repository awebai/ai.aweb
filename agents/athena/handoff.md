...
- Current heads: aweb main `605f356`; AC main `0caaefc4`. AC `.3` landed at `b0e82553`; AC `.4` merged after it at `f52f5481`; final `.5` cleanup at `d80fe410`; Grace-blocker patch at `0caaefc4`. No tag/deploy/release.
...
- AWID row-disposition follow-up: Athena landed aweb main `605f356` (`Fail closed before dropping legacy address visibility`). It makes `awid/src/awid_service/migrations/003_drop_address_reachability.sql` refuse to drop `reachability` / `visible_to_team_id` while any active `public_addresses` row is non-neutral (`reachability != 'public'` or `visible_to_team_id IS NOT NULL`). This encodes the disposition as fail-closed: hidden/limited active rows are not silently widened; an operator must explicitly normalize or retire them before deploy. Validation: aweb diff-check clean; `make test-awid` -> 168 passed. Grace narrow review requested in mail `e9e4a27a-cfdc-4645-8ca0-b8f3e7997210`. Do not ask Hestia for no-deploy release-ready until Grace accepts this gate.
