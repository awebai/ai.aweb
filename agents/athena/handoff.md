...
- Current heads: aweb main `bf8b4e4`; AC main `f52f5481`. AC `.3` landed at `b0e82553`; AC `.4` merged after it at `f52f5481`. No tag/deploy/release.
...
- `aweb-aapj.5` final AC cleanup landed on AC main at `d80fe410`. It removes the release-blocking AC public/static docs/site old vocabulary and frontend adapter/API `lifetime`/`address_reachability`/`identity_type` residue, updates e2e old flags to `--global`/`identity_scope`, and tightens the frontend vocab gate. Validation: diff-check clean; frontend aapj vocab gate OK; frontend vitest green (38 files / 194 tests; existing jsdom `window.scrollTo` stderr only); dashboard build + frontend build green; py_compile touched backend Python green. Final reports: `/tmp/aapj5-final-20260521133557/raw.txt` and `strict.txt`. Remaining strict hits are classified as compatibility/audit/history boundaries. Grace has been asked for the requested thorough final review of aweb `bf8b4e4` + AC `d80fe410`. Do not ask Hestia for no-deploy release-ready until Grace is clean and AWID hidden/limited row disposition is resolved.
