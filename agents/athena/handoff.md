...
- Olivia `.6` branch `olivia-aapk-3` reached `2daf7688` with AgentDetail/global picker, hosted MCP picker, CLI picker, deps fix, and Juan-approved spacing. Athena initially picked a split path, but Juan overruled it: do **not** hide/remove the CLI picker and do **not** merge partial AC to main. The whole AC branch stays unmerged until `aweb-aapl.7` lands and CLI global creation works end-to-end. Athena created P0 `aweb-aapl.7` assigned to Mia for Go CLI `aw init --inbound-mode <open|contacts_only>` support. Mia stacked `.7` on branch `origin/mia/aapl4-remove-third-mode`: `c2164667` fixed API-key/dashboard path; `6f990e16` threads guided hosted and fail-fasts BYOD. Athena reviewed `6f990e16`: aweb/CLI side now fixes silent-ignore, but `.7` is still blocked because AC main `ee045916` rejects `inbound_mode` on `/api/v1/onboarding/cli-signup` (`CliSignupRequest` extra=forbid, no field, no pass-through). Need companion AC patch/tests before Olivia live-verifies. Generated commands must use CLI hyphen spelling `--inbound-mode contacts-only`, while wire/API remains `contacts_only`.
