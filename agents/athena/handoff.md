...
- Current heads: aweb main `fa1041c`; AC main `9104ffc2`. aweb includes AWID row-disposition regression `d300b33`, Peter's CLI workspace-init request-shape fix `b215d23`, Peter's CLI init JSON output cleanup `af317d3`, and Athena's no-`Lifetime` connectOutput tightening `fa1041c`. AC `.3` landed at `b0e82553`; AC `.4` merged after it at `f52f5481`; final `.5` cleanup at `d80fe410`; Grace-blocker patch at `0caaefc4`; Hestia validation-failure patch at `b1f6277e`; two-service e2e fixture patch at `74ab465c`; e2e contract cleanup at `3c97b4d3`; two-service sibling-source boundary fix at `bf9206b5`; no-sync preservation fix at `4034f044`; browser selector + guard at `dccfc8d0`; TS guard tightening at `492b3d33`; Peter AC expectation at `1ea76dd6`; double-quoted TS guard at `7eb391bd`; hosted roster refetch fix at `d77e0934`; enriched dashboard agents roster fix at `9104ffc2`. No tag/deploy/release.
- Hestia's validation-only sibling-source chain for aweb `fa1041c` + AC `9104ffc2` is green across all 6 phases: release-verify-* PASS; `test-backend-aweb-local` PASS 1435/1435; `test-frontend` PASS 195/195; `release-build-image` PASS; `test-two-service` PASS 15/15; `test-cloud-user-journeys` PASS 11/11 browser tests + ALL PASSED 263 tests + PASS browser journey. Full log: `/tmp/ac-aapj-sibling-chain-9104ffc2.log`. Boundary preserved: no tags, pushes, deploys, bumps, migrations, or row mutations. Hard hold remains until Juan explicitly clears release actions. Sofia has been asked to route the green summary/clearance question to Juan because Juan is not an aw recipient in `default:aweb.ai`.
- New pre-release blocker `aweb-aapk`: Juan clarified the onboarding auto-provision change blocks release. Direction via Olivia (conversation `a3e9c853-7359-4b20-9a5b-26680cf40816`): drop the post-signup "create your first team" onboarding step. Register should auto-provision a personal team from username/slug and land on that dashboard; BYOT/explicit team creation remains later. Username availability must check both AC account username and AWID team namespace slug availability. Olivia also found small dev-mode issues: `make dev-awid` missing `ENVIRONMENT=development`, and reserved `.test`/`.example` emails rejected with raw validator copy. Athena created/routed: `aweb-aapk.1` Peter, `aweb-aapk.2/.3` Mia, `aweb-aapk.4` Grace. Olivia stays free for Juan walkthroughs. `.1` is now closed at aweb `b7e2192`: AC should call unauthenticated/read-only `GET /v1/namespaces/{domain}/teams/{name}`; 404 means free/deleted, 200 means active/taken, 422 invalid. Intended personal team shape is `default:{username}.{managed_namespace_base_domain}`. Peter flagged AC web register drift: current `/register` appears to create `{username}:{username}.{base}` by deriving AWID team name from OSS team slug=username; Mia must align to `default`. Mia found `/register` creates user+org in one tx then team outside; Athena rejected soft/hard deleting user/org after namespace registration because AWID namespace/controller state must be preserved. Directed shape: add idempotent helper to ensure/recover personal default team, callable from `/register` and recovery point (`/auth/me`, onboarding status, or small setup completion endpoint), preserve org controller material, test idempotency and recovery after transient team creation failure. Juan added setup-progress-log UX: show running/completed steps for creating organization, creating namespace, registering team in AWID using actual canonical values, keep the log visible if useful, then offer first CLI workspace or hosted MCP identity without reintroducing first-team creation.
