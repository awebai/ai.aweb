# Athena Handoff
Last updated: 2026-05-23 10:26 GMT
...
- Olivia merged hosted ID-space epic + consent picker directly to AC main per Juan override; AC main is `7365cc03`. Grace reviewed/cleared the ID-space fixes on main: create/connect, pending connect storage, Manage identity link, BYOT pending response, dashboard replace navigation, and `admin_support.py` now sourcing `replacement_agent_id` from `replacement.agent_id` with support audit `agent_id` explicit and `identity_id` compatibility-only. Remaining caveat: two-service e2e is still gate evidence before release closure. Athena fetched/fast-forwarded local AC main and told Olivia no backout.
...
- Packaging correction: Dave's `fc9e8d0` made `@awebai/channel-core` a separate runtime dependency/artifact for `@awebai/claude-channel` and `@awebai/pi`; Juan decided that was a release-risk mistake. Athena reverted it on aweb main `65c3cbf`, so channel and pi bundle channel-core again and no separate `@awebai/channel-core` npm publish is required. Dave later pushed aweb `aa7a627` team-bootstrap refactor; Athena ran focused `go test ./cmd/aw -run 'TeamBootstrap|HostedSignup|Init|Connect' -count=1` and it passed. Hestia + Dave notified; previous channel-core-publish-order warning is superseded.
- Grace final `.3` structural review passed for aweb-aapl. Athena closed `.3` and `.6`; all aapl tasks `.3/.4/.5/.6/.7` are closed. Grace then fixed Olivia's separate CLI global-init registry mismatch on AC main `3f6ab955`: generated global CLI commands include `AWID_REGISTRY_URL` from `/api/v1/discovery`, while local commands omit it. Olivia reran the exact clean-temp-dir live check and passed: `aw init --global --name foxtrot --inbound-mode contacts-only` connected and backend persisted `inbound_mode='contacts_only'`. Grace also patched valid legacy-cert compatibility hardening on aweb `f4f3872` to preserve already-signed legacy `lifetime` cert wire shape when re-encoding; Dave reran clean-temp hosted `aw init` against app.aweb.ai with aw rebuilt from `f4f3872` and passed. Hestia was updated. Hestia's no-deploy Phase 3 found a real AC frontend build blocker: `@aweb/dashboard` package root did not export `AgentInboundMode` / `AgentInboundModeState`. Athena patched AC main `4d14fe85` (`Export dashboard inbound-mode types`) and validated focused frontend tests (20 passed) plus `npm --prefix frontend run build`. Hestia's no-deploy chain next found a stale Phase 6 cloud-user-journey fixture expecting the pre-aapk `/onboarding` stop. Athena patched AC main `ec0fcf04` so the e2e helper accepts/captures direct `/teams/<team_id>` auto-provision routes and updated the aapk assertion; validation: diff-check, ContactsTab focused vitest (2 passed), Playwright list of all 11 cloud journey tests. Release/cutover is in progress after Juan clearance. Hestia completed restore to AC `v0.5.45`; row counts matched and live federated mail smoke works. New blockers after restore: AWID `0.5.8` deploy fails intentionally on `003_drop_address_reachability.sql` because 43 active `public_addresses` rows still have legacy non-neutral reachability metadata; Grace prepared validated all-normalize AWID artifacts, but then issued a hold: do **not** apply blanket normalization until Juan/Athena make an explicit per-class/per-row disposition using the decision CSV/report. Hestia was told to stand down on AWID mutation; AC immutability rerun and `aw-v1.25.0` remain held. AC post-cutover `release-verify-migration-immutability` also exposed a script path bug; Athena patched AC main `aeccb3dc` so the gate resolves by `(schema,module_name,filename)` and canonical aweb rows point at installed aweb package while overlay rows point at `migrations/aweb_overlay/`. Hold `aw-v1.25.0` push until AWID 0.5.8 and AC immutability pass normally.
