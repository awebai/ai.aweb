...
- **Published artifacts:** aweb `1.25.0` (PyPI + npm `@awebai/aw`), awid-service `0.5.8`, awid `0.5.8` (GHCR + api.awid.ai), AC `v0.5.45`, `@awebai/claude-channel 1.4.4` then `1.4.5`, `@awebai/pi 0.1.1` then `0.1.2`. `@awebai/channel-core` is bundled into channel/pi, not a separate runtime dependency.
