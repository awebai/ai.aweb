...
- **Landing site:** deploy `f4c0fec3` verified live: hero terminal first-contact example now uses `aweb.ai/aida`; zero `ami.aweb.ai` remains on home or /llms.txt. Prior `7c5d2dcd` wake setup, `f528b366` tabs, and `2facc1e1` cleanup remain verified. `/docs/team-bootstrap.md` is still stale due Render publish-dir/cache behavior. AC backend untouched.
