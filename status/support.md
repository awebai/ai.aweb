# Support Status
Last updated: 2026-06-07 (aapz EPIC COMPLETE — Wave 1 awid 0.5.10 → Wave 2 aweb/aw 1.26.8 → Wave 3 AC v0.5.60 + site canonical → Wave 4 channel 1.4.12 + skills 0.2.12 + Pi 0.1.20 + marketplace rematerialized)
...
- **aapz EPIC COMPLETE — Wave 1+2+3+4 verified-live end-to-end (Hestia `ad5c7b9b` Wave 2 + `0a21d37c` Wave 3 + `bf5f579e` Wave 4, 2026-06-07).** Four-wave release wave closed across awid 0.5.10 (atomic AWID identity+address claim primitive) → aweb/aw 1.26.8 (aw agents repo-root lifecycle replacing `aw team bootstrap`) → AC v0.5.60 + Olivia site canonical at aweb.ai/orchestration/ + /docs/team-bootstrap/ → channel 1.4.12 + skills 0.2.12 + Pi 0.1.20 + claude-plugins marketplace.json rematerialized to 1.4.12 + 0.2.12. **Customer-facing support impact**: (1) `aw team bootstrap` → `aw agents bootstrap` is the canonical CLI shape now (the `aw team` command group is NOT wired to rootCmd on 1.26.8; pasting it returns "command not found"); landing site copy updated this cycle. (2) Channel skill 1.4.12 ships `aw init --setup-channel` instead of `aw run claude` — corroborates my Customer-Facing Default #2 ("do not recommend `aw run` for Claude Code customers") from the skill side. (3) Bundled skills carry canonical `aw agents bootstrap` + `aweb-team-coord-worktrees` content; customer upgrade path is `/plugin marketplace update awebai-marketplace && /plugin update aweb-channel@awebai-marketplace`. **Runbook check**: zero `aw team bootstrap` / `aweb-team-dev-review` refs in `docs/support/runbook.md`. The three `aw run claude` mentions (lines 468, 546, 712) are all negative-framing ("don't use") and survive — they tell customers not to use it, which is the intent. Hestia banked the same lesson: "negative-framing mentions of deprecated commands survive grep-based forbidden-string checks; verify-intent not just verify-presence." **#27 refinement (CLI surface rename)**: implementation file is still `cli/go/cmd/aw/team_bootstrap.go` (internal var names kept the old `teamBootstrap*` prefix), CLI surface is `aw agents <subcommand>`. Source-grep the file for implementation truth; recommend `aw agents bootstrap` to customers. **Watch state**: customer who pastes `aw team bootstrap` from a stale tutorial / blog post / Reddit answer is the realistic miss path here; flag the rename immediately when seen.
...
- **`juan.aweb.ai/olivia` Pi-README cold-reader ask — CLOSED + SHIPPED across Pi 0.1.18 → 0.1.19 (Hestia `9d1ff678` 2026-06-06; Hestia `ce7ab07e` 2026-06-07).** Olivia (dev-team agent) opened a chat asking two questions about Pi's package README copy: (1) one-sentence no-internal-vocab description, (2) lead with "what is aweb" or assume click-through. Engaged directly — customer-comprehension is my lane, and the question was about cold-Pi-user-readability, not engineering coordination. Gave a draft sentence ("Lets your Pi send and receive messages with other AI agents on an open network — and wakes it up the moment one arrives.") + structure recommendation (brief context graf + link out + dive in) + flagged the framing-lock discipline (customer-facing package copy goes through Sofia before ship). Olivia took the structure to Juan (he authored the final copy directly; Sofia/Athena framing chain explicitly bypassed by Juan per Hestia, not a deviation from discipline — explicit-author override). **Two-pass refinement worth banking** for next customer-facing one-liner draft, since both passes changed something my draft got wrong: (1) Lead with the highest-level benefit-verb, then detail with the mechanism. 0.1.18 refined "send and receive messages" (mechanism-led) → "communicate ... send and receive messages" (benefit-verb-led, mechanism in detail clause). (2) Cover the full capability surface, not just the headline mechanism. 0.1.19 refined em-dash + "and" → colon-led tri-clause AND added "join agent teams" — a real Pi capability my first draft elided entirely. The shape that ships: `<benefit-verb-lead>: <mechanism>, <event>, <new surface>.` Distribution loop tight: customer-readability ask → drafted copy → 0.1.18 ship in ~24h → 0.1.19 refinement next day.
