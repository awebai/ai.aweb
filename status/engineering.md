...
- **aweb-aapj.3 — AC backend/schema/API cleanup**: assigned to Mia and actively driven. Latest fetched branch `origin/mia/aapj-3-phase-a` at `3389db12`. Athena live preflight found remaining normal-surface blockers in AC team-auth/cert/scope/agent-addressing/init/spawn/teams/admin support paths; blocker list sent to Mia. Peter is live on refreshed `.9` blocker map; Dave is live on `.10` gate preflight.
...
- Track Mia `.3` to branch-ready; final `.5` criteria are tightened: no derived old fields from canonical state, even under support/audit legacy names. Dave/Peter are no longer idle and are producing current blocker reports.
