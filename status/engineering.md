...
1. **Global/local simplification aweb server 1.24.4 is PyPI-published; AC v0.5.45 is proceeding if green; npm CLI publish is broken.** Hestia gate at aweb `16a062a`/`1.24.4` went green including Phase 11c prompt rejection; tags `server-v1.24.4` + `aw-v1.24.4` were pushed at release commit `22d3ccf`; PyPI `aweb==1.24.4` is live. npm `@awebai/aw` failed on platform package auth-like 404, so CLI/npm remains `1.24.3` and must not be claimed as 1.24.4. Athena told Hestia to proceed with AC `v0.5.45` if AC gate is green because AC uses PyPI and the fix is server-side. No tags/deploys by Athena.
...
- **aweb-aapg — ship-grade simplification epic**: aweb server release advanced. Hestia's first gate at `ba3e5e5` caught Phase 11c timeout on `contacts_only` refusal; Athena patched cached-body ASGI replay at `16a062a`; Grace approved; Hestia gate green; release commit is aweb `22d3ccf` with `server-v1.24.4` + `aw-v1.24.4` tags. PyPI `aweb==1.24.4` is live. npm `@awebai/aw` publish failed, so CLI npm stays `1.24.3`; handle as follow-up and don't claim CLI 1.24.4. AC `v0.5.45` can proceed if AC gate green with backend lock at `aweb==1.24.4` and live `contacts_only` prompt-4xx smoke.
...
- Athena authored release-blocker middleware fix at aweb `16a062a`; merged/pushed to main. Grace approved; Hestia full Docker gate passed and PyPI published at server 1.24.4. npm CLI publish failure remains an ops/distribution follow-up.
...
- **Global/local simplification AC gate input is AC `30b1b761` repinned to PyPI `aweb==1.24.4`.** aweb server gate passed and PyPI 1.24.4 is live from release commit `22d3ccf`. npm `@awebai/aw` 1.24.4 failed publish; do not claim CLI/npm 1.24.4 until fixed. Hestia should proceed with AC only if `make ship` green and live `/health` shows `aweb_version=1.24.4`.
...
- **Global/local release risk is now AC-live verification plus npm follow-up.** aweb server gate proved Phase 11c returns prompt rejection and PyPI 1.24.4 is live; AC still must prove app.aweb.ai runs `aweb_version=1.24.4` and live `contacts_only` prompt-4xx/no-delivery. npm `@awebai/aw` publish failed, so CLI/npm 1.24.4 is not available. Production hidden/limited rows stay blocked by `.2` code until explicit owner/operator normalization; do not mutate row groups without routed approval.
...
## Next checks
- Watch Hestia AC `v0.5.45` gate/deploy for `aweb-aapg`: backend lock must use `aweb==1.24.4`, `/health` must show `v0.5.45` + `aweb_version=1.24.4`, and live `contacts_only` prompt-4xx/no-delivery smoke must pass. Track npm `@awebai/aw` 1.24.4 publish failure as follow-up; do not claim CLI/npm 1.24.4. Do not tag/deploy from Athena.
