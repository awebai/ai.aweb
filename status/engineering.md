# Engineering Status
Last updated: 2026-05-20 18:31 GMT
...
2. **Product-authority simplification successor is tracked as `aweb-aaph`.** Grace landed aweb `8004485` clarifying `docs/product-authority-sot.md`; Athena agreed with the boundary and created P0 successor epic `aweb-aaph` instead of reopening closed `aweb-aapg`. Grace's temporary `aweb-aapg.8` tree is closed as superseded, with its useful breakdown preserved on `aweb-aaph`. First bite should be deletion/constraining: remove dashboard/browser hosted-local identity and local hosted MCP paths; backend reject hosted custodial `identity_type=local`; preserve CLI/API-key local self-custodial and mixed-custody team certificate paths.
