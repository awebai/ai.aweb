# Engineering Status
Last updated: 2026-05-21 17:35 GMT
...
- AC main is `4034f044`: Mia correctly blocked `bf9206b5` because plain `uv run` would re-sync away the sibling overlay before pytest. `4034f044` keeps `test-two-service-up` on `use-aweb-local` and changes both host bootstrap calls plus pytest to `uv run --no-sync ...`. Mia follow-up review requested before Hestia rerun.
...
- Mia has follow-up review request for AC `4034f044` (conversation `288d6b29-7a28-47d9-8684-1376e0970958`, latest Athena message `0fc44b5e-9d15-4dc7-966b-95a13470759e`). Hestia has been told AC `4034f044` is patched but review pending (company conversation `96317ca9-a823-40ad-8216-29670533d673`, latest Athena message `6b2bedde-1106-45b7-9be4-6a0d557d6377`).
- Current heads: aweb main `b215d23` (includes AWID row-disposition regression `d300b33` and CLI identity-scope request fix `b215d23`); AC main `4034f044` (includes `.3` `b0e82553`, `.4` merge `f52f5481`, final cleanup `d80fe410`, Grace-blocker patch `0caaefc4`, validation patch `b1f6277e`, two-service fixture patch `74ab465c`, e2e contract cleanup `3c97b4d3`, two-service sibling-source boundary fix `bf9206b5`, and no-sync preservation fix `4034f044`).
...
## Next checks
- Wait for Mia review of AC `4034f044`; if passed, ask Hestia to rerun sibling-source no-publish validation for aweb `b215d23` + AC `4034f044`.
