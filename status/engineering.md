...
1. **Global/local simplification release is held until `aweb-aaph` is complete.** Hestia gate at aweb `16a062a`/`1.24.4` went green; PyPI `aweb==1.24.4` is live; npm CLI remains `1.24.3`. AC `deec72c7` failed full backend due stale `awid-service==0.5.5` and a remaining reachability-call mismatch. Hestia published awid-service `0.5.7`; Athena folded AC release-compat into review branch `athena/aaph-1-2-review` commit `c86b956b` (lock `awid-service==0.5.7`, remove `reachability=` caller, helper compatibility). No tags/deploys by Athena.
2. **Product-authority simplification `aweb-aaph` is active now.** Juan clarified release is held until this is done; do not wait behind AC recovery. AC review branch `awebai/ac` `athena/aaph-1-2-review` head `862c7e33` is pushed. Main `.1/.2` commit `159ddd7b` removes dashboard/browser hosted-local identity path and rejects hosted custodial `identity_type=local|ephemeral`; validation on branch: backend focused 68 passed, frontend focused 9 passed, frontend build passed, diff-check passed, stale hosted-local grep no hits. Grace review requested. `.3` assigned to Peter. `.6` Dave active on docs/copy.
...
- **aweb-aaph — product-authority simplification epic**: active release blocker. Athena leads implementation with Dave/Peter; Grace reviews/boundary-checks slices. `.1/.2` review branch is AC `athena/aaph-1-2-review` head `862c7e33` (primary deletion commit `159ddd7b`, release-compat `c86b956b`, docs copy `862c7e33`); Grace review pending. `.3` assigned to Peter from that branch. `.6` Dave active on docs/copy. `.4/.5/.7` not routed yet. Scope: no hosted-local browser/MCP product shape, hosted custodial creation global-only, local self-custodial CLI/API-key preserved, mixed-custody teams through current team authority certs, BYOT custodial pending/import fail-closed.
...
## Next checks
- Drive `aweb-aaph` to completion now: wait for Grace review on `.1/.2` branch, review Peter `.3`, continue Dave `.6`, then route `.4/.5` mixed-custody proofs and `.7` final conformance. Do not claim CLI/npm 1.24.4. Do not tag/deploy from Athena.
