# Engineering Status
Last updated: 2026-05-20 00:05 GMT
...
- **aweb-aapg — ship-grade simplification epic**: open P0. `.1` federation v1 compatibility tolerance is closed/landed at aweb `e4ff4e9`. `.4` messaging-policy removal is closed/landed at aweb `8325b713` + AC `ed2218ad` after Athena review and Grace boundary approval. `.2` hidden/limited AWID row policy is unassigned/product+ops. `.3` delivery-origin rollout is blocked on route-level origin redesign. `.5` docs convergence unassigned. `.6` minimal e2e proof assigned Peter but blocked on `.2-.5,.7`. `.7` AC product cleanup remains open for any post-`.3/.4` product cleanup. Release remains held until epic closes or Juan chooses a different stance.
...
## Next checks
- Lead `aweb-aapg`: `.4` is landed/closed. Next simplification focus is `.2` hidden/limited AWID row disposition and `.3` delivery-origin/route model. Do not let Hestia tag/deploy current aweb/AC heads as-is. Pull Grace back for boundary review as needed.
