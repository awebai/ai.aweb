...
1. **Global/local simplification is not ship-ready; `aweb-aapg` P0 epic tracks making it actually simpler.** `aweb-aapg.1` federation v1 compatibility tolerance is closed at aweb `e4ff4e9`. `aweb-aapg.4` messaging-policy removal is closed and landed after Athena+Grace review at aweb `8325b713` and AC `ed2218ad`: runtime delivery auth now reads `inbound_mode=open|contacts_only`, legacy `messaging_policy` is quarantined to migrations/tests, and stale AC `access_mode` delivery UI is removed. Active work is `.3` route-level delivery-origin redesign: Grace approved the boundary, Peter sent inventory/design, and Athena cleared him to implement with gates. `.2` hidden/limited-row policy is release-time disposition (hidden/limited rows become private/blocked for first contact unless explicitly converted; no silent widening). Remaining release blockers: `.3`, `.2`, full docs convergence, remaining AC cleanup, and final minimal e2e proof. No tags/deploys.
...
- **aweb-aapg — ship-grade simplification epic**: open P0. `.1` federation v1 compatibility tolerance is closed/landed at aweb `e4ff4e9`. `.4` messaging-policy removal is closed/landed at aweb `8325b713` + AC `ed2218ad` after Athena review and Grace boundary approval. `.3` route-level delivery-origin redesign is in progress with Peter; Grace approved the architecture boundary and Athena cleared implementation after Peter's call-site inventory/design checkpoint. Gates: address route authority for first contact, stored participant route authority for continuation, no bare external `did:aw` first-contact route discovery, contacts do not route, delete unshipped identity-level `delivery_origin` artifacts or stop/report shipped dependency, include same-DID/two-address/two-origin and fail-closed tests. `.2` hidden/limited AWID row policy is release-time/product+ops disposition: hidden/limited rows become private/blocked for first contact unless explicitly converted; no silent widening. `.5` docs convergence unassigned. `.6` minimal e2e proof blocked on `.2-.5,.7`. `.7` AC product cleanup remains open for any post-`.3/.4` product cleanup. Release remains held until epic closes or Juan chooses a different stance.
...
- **Global/local simplification is release-blocked.** aweb main is now `8325b713` and AC main is `ed2218ad` with `.4` landed, but the wave still must not ship. Remaining blockers: Hestia's real AWID audit found 43 hidden/limited rows; delivery-origin/route authority model needs correction because one `did:aw` may have multiple address routes/origins; `.5` full docs convergence, `.6` e2e proof, and `.7` remaining AC cleanup remain open.
...
- **Global/local transition risk**: current main demotes reachability from resolver authority while production still has 43 hidden/limited AWID rows. Old->new federation wire compatibility is fixed at aweb `e4ff4e9`, and messaging-policy delivery authority removal is landed at aweb `8325b713` + AC `ed2218ad`; privacy/migration, delivery-origin route model, full docs convergence, remaining AC cleanup, and e2e proof remain unresolved. Treat current heads as non-ship transition artifact until `aweb-aapg` closes or Juan chooses a different stance.
...
## Next checks
- Lead `aweb-aapg`: `.4` is landed/closed. Active simplification focus is `.3` route-level delivery-origin implementation by Peter under the Grace-approved boundary. Manually check comms because event delivery is replaying stale mail. Do not let Hestia tag/deploy current aweb/AC heads as-is. Pull Grace back for boundary review on Peter's `.3` diff/packet as needed.
