...
1. **Global/local simplification epic: `aweb-aapf.3` reviewed, not approved; simplicity gate tightened.** `aweb-aapf` captures Juan's target model: global = `did:aw` + AWID identity delivery origin; local = renamed ephemeral `did:key`, not globally first-contactable, replyable in established context. Athena approved/closed `.1` and `.2`, then reviewed Peter's `.3` commit `0e06284` and blocked it. Juan pushed back that local agents already had ephemeral reply behavior; Athena tightened the gate: preserve/simplify that behavior, do not invent a local mini-registry/capability protocol unless a concrete exploit requires it, and prove the epic with deletion. Conversation_id may be an index into local conversation/session state, but never routing authority: sender must be authenticated as a stored participant and route must come from stored participant/session state. `aweb-aapf.7` is assigned to Grace as a separate test-contract/prune pass; Grace completed inventory-only with no edits and identified stale e2e Phase 12e, federation Phase 5 private auth, private reachability envelope/tests, messages/chat/MCP private-address/conversation-auth clusters, and CLI reachability fixtures. CLI/channel/AC remain gated.
...
- **aweb-aapf — global/local identity simplification**: Peter assigned epic + eight dependent subtasks. `.1` approved/closed at `4b51af1`; `.2` approved/closed at `4509c9f`. `.3` commit `0e06284` reviewed/not approved; tightened direction is to preserve/simplify old ephemeral reply behavior, not grow a local route mini-protocol. `.7` assigned to Grace for second-developer test-contract/e2e pruning; `.8` deletion is the proof point.
