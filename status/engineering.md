...
- Do not release yet: Juan asked for tests only; hard hold remains on tag/deploy/publish/version bump/prod migration/prod row mutation until explicit clearance.
- Known release caveats: npm `@awebai/aw` remains `1.24.3`; do not claim npm/CLI `1.24.4`. AWID health still needs observed `0.5.7`.

## Risks
- **Release-clearance risk**: validation is green, but release actions remain blocked until Juan explicitly clears them.
