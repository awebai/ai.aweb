...
1. **Global/local simplification is not ship-ready; `aweb-aapg` P0 epic tracks making it actually simpler.** `aweb-aapg.1` federation v1 compatibility tolerance is closed at aweb `e4ff4e9`. `aweb-aapg.4` messaging-policy removal is closed at aweb `8325b713` + AC `ed2218ad`. `aweb-aapg.3` route-level delivery-origin redesign is now closed/landed at aweb `50def8e` + AC `177415e`: address route is first-contact authority; `did:aw` is identity binding not delivery route; bare external `did:aw` first-contact fails closed; identity-level delivery-origin endpoint/CLI/migrations/helpers/provisioning removed as unshipped transition artifacts; stored-route federation continuation validates target current key. `.2` hidden/limited-row policy is release-time disposition (hidden/limited rows become private/blocked for first contact unless explicitly converted; no silent widening). Dave owns `.5` docs convergence against landed main. Remaining release blockers: `.2`, `.5`, `.6`, `.7`. No tags/deploys.
...
- **aweb-aapg — ship-grade simplification epic**: open P0. `.1` federation v1 compatibility tolerance is closed/landed at aweb `e4ff4e9`. `.4` messaging-policy removal is closed/landed at aweb `8325b713` + AC `ed2218ad` after Athena review and Grace boundary approval. `.3` route-level delivery-origin redesign is closed/landed at aweb `50def8e` + AC `177415e` after Athena review and Grace boundary approval. Landing validation: aweb diff-check, docs regression, new stored-route wrong-key regressions + federation envelope 16 passed, namespace command/help focused tests passed; prior focused server 221 and AC backend 72 passed. `.2` hidden/limited AWID row policy is release-time/product+ops disposition: hidden/limited rows become private/blocked for first contact unless explicitly converted; no silent widening. `.5` docs convergence assigned to Dave and in progress, docs-only against landed main. `.6` minimal e2e proof blocked on `.2/.5/.7`. `.7` AC product cleanup remains open for residual cleanup after `.3/.4`. Release remains held until epic closes or Juan chooses a different stance.
...
- **Global/local simplification is release-blocked.** aweb main is now `50def8e` and AC main is `177415e` with `.3` and `.4` landed, but the wave still must not ship. Remaining blockers: Hestia's real AWID audit found 43 hidden/limited rows requiring `.2` disposition; `.5` full docs convergence (Dave in progress), `.6` e2e proof, and `.7` remaining AC cleanup remain open.
...
## Next checks
- Lead `aweb-aapg`: `.3` and `.4` are landed/closed. Active simplification focus is Dave's `.5` docs convergence and `.2` release-time hidden/limited row disposition. Do not let Hestia tag/deploy current aweb/AC heads as-is. Pull Grace back for boundary review on docs/e2e if the contract drifts.
