# Engineering Status
Last updated: 2026-05-20 18:50 GMT
...
1. **Global/local simplification AC v0.5.45 gate halted; awid-service release boundary was missed.** Hestia gate at aweb `16a062a`/`1.24.4` went green; tags `server-v1.24.4` + `aw-v1.24.4` were pushed at release commit `22d3ccf`; PyPI `aweb==1.24.4` is live. npm `@awebai/aw` failed, so CLI/npm remains `1.24.3`. AC `deec72c7` gate failed full backend (`72 failed, 1362 passed`) because the lock still had stale `awid-service==0.5.5` and AC still has a remaining reachability-call mismatch. Athena pushed aweb `2cd8082` preparing `awid-service==0.5.7` (`make test-awid` 167 passed). AC remains held; no tags/deploys by Athena.
2. **Product-authority simplification successor is tracked as `aweb-aaph`.** Grace landed aweb `8004485` clarifying `docs/product-authority-sot.md`; Athena agreed with the boundary and created P0 successor epic `aweb-aaph` instead of reopening closed `aweb-aapg`. Grace's temporary `aweb-aapg.8` tree is closed as superseded, and live `aweb-aaph.1`-`.7` child tasks now exist. Juan explicitly wants Athena to lead implementation to completion with Dave/Peter and Grace reviewing/boundary-checking. Leave tasks unassigned until awid-service/AC recovery is clean; first bite is `.1` + `.2`: remove dashboard/browser hosted-local identity and local hosted MCP paths, and backend reject hosted custodial `identity_type=local`.
3. **Mail/channel event delivery is replaying stale messages.** Manual comms check is required until the channel is fixed: inspect `aw chat pending`, `aw mail inbox --limit <n>`, task comments, and message IDs/timestamps before acting. Repeated `.4`/pre-pivot Grace briefs and ontology/company-graph messages are stale unless there is fresh `.3` content.
...
- **aweb-aapg — ship-grade simplification epic**: aweb server release advanced but AC gate is blocked. Hestia's first gate at `ba3e5e5` caught Phase 11c timeout; Athena patched at `16a062a`; Grace approved; Hestia gate green; release commit is aweb `22d3ccf` with `server-v1.24.4` + `aw-v1.24.4` tags. PyPI `aweb==1.24.4` is live; npm `@awebai/aw` publish failed, so CLI npm stays `1.24.3`. AC `deec72c7` failed full backend due stale `awid-service` lock / missed awid-service release plus remaining AC reachability mismatch. Athena pushed aweb `2cd8082` for `awid-service==0.5.7`; Hestia must publish/release awid-service before AC can lock/fix/rerun.
- **aweb-aaph — product-authority simplification epic**: new P0 successor created by Athena after Grace landed product-authority SOT clarifications at aweb `8004485`; live children `.1`-`.7` exist and are unassigned until release recovery completes. Athena leads implementation with Dave/Peter; Grace reviews/boundary-checks slices. Scope is code/UX deletion and exact authority-boundary tests: no hosted-local browser/MCP product shape, hosted custodial creation global-only, local self-custodial CLI/API-key preserved, mixed-custody teams through current team authority certs, BYOT custodial pending/import fail-closed. `.6` is broad communication cleanup across OSS docs, AC site/static docs, hosted app/dashboard copy, agent-facing docs, tutorials, references, support docs, generated/static docs.
...
- Athena authored release-blocker middleware fix at aweb `16a062a`; merged/pushed to main. Grace approved; Hestia full Docker gate passed and PyPI published at server 1.24.4. Athena also authored aweb `2cd8082` to prepare awid-service 0.5.7 after AC gate exposed the missed awid-service release boundary. Local AC WIP exists but is not pushed pending PyPI awid-service 0.5.7.
...
- **Global/local simplification AC gate is held.** AC `deec72c7` (v0.5.45 bump to `aweb==1.24.4`) is pushed but red; no AC tag/deploy. Next required release input is awid-service `0.5.7` published from aweb `2cd8082`, then AC fix/lock to `awid-service>=0.5.7`, full backend green, and only then AC `make ship`.
...
- **Global/local release risk is now awid-service + AC full-suite recovery.** aweb server gate proved Phase 11c returns prompt rejection and PyPI 1.24.4 is live, but AC full backend exposed stale awid-service and remaining reachability mismatch. AC must not ship until awid-service 0.5.7 is published/deployed as needed, AC full backend/make ship are green, app.aweb.ai runs `aweb_version=1.24.4`, and live `contacts_only` prompt-4xx/no-delivery passes. npm `@awebai/aw` publish failed, so CLI/npm 1.24.4 is not available. Production hidden/limited rows stay blocked by `.2` code until explicit owner/operator normalization; do not mutate row groups without routed approval.
...
## Next checks
- Watch Hestia awid-service `0.5.7` release from aweb `2cd8082`, then finish AC fix/lock/rerun for `aweb-aapg`. After release recovery, brief/route `aweb-aaph` first implementation slice. Do not claim CLI/npm 1.24.4. Do not tag/deploy from Athena.
